...
#define CALL_LUA_HOOK(func){ \
	LuaHookMap * pMap = sScriptMgr.GetLuaHookMap(); \
	if(pMap != NULL && pMap->size()) { \
	for(LuaHookMap::iterator itr = pMap->begin(); itr != pMap->end(); itr++) if( (*itr) != NULL) (*itr)->func; } }
#endif
...
enum ServerHookEvents
{
	...
	SERVER_HOOK_EVENT_ON_LOOT				= 35,
	SERVER_HOOK_EVENT_ON_OBJECTLOOT			= 36,
	SERVER_HOOK_EVENT_ON_POST_LEVELUP		= 37,
	SERVER_HOOK_EVENT_ON_CONTIENT_CREATE	= 38,
...
typedef bool(*tOnPostLevelUp)(Player* pPlayer);
...
#ifdef ENABLE_LUA_HOOKS
typedef set<LuaHookInterface *> LuaHookMap;
#endif
...
#ifdef ENABLE_LUA_HOOKS
	void register_lua_hook(LuaHookInterface*);
	void restart_lua_scripts();
	void unregister_lua_hook(LuaHookInterface*);
	ARCEMU_INLINE LuaHookMap * GetLuaHookMap() { return lua_hookmap; }
#endif
...
#ifdef ENABLE_LUA_HOOKS
class SERVER_DECL LuaHookInterface
{
protected:
	/* For LuaEngineMgr global bool result check */
//	typedef HM_NAMESPACE::hash_map<uint32,bool> BoolResult;
	typedef unordered_map<uint32,bool> BoolResult;
	BoolResult holdbools;
public:
	friend class ScriptMgr;
	BoolResult GetBoolResultMap() { return holdbools; }

	virtual bool lOnNewCharacter(uint32 Race, uint32 Class, WorldSession * Session, const char * Name) { return false; }
	virtual void lOnKillPlayer(Player* pPlayer, Player* pVictim) {}
	virtual void lOnFirstEnterWorld(Player* pPlayer) {}
	virtual void lOnEnterWorld(Player* pPlayer) {}
	virtual void lOnGuildCreate(Player* pLeader, Guild * pGuild) {}
	virtual void lOnGuildJoin(Player* pPlayer, Guild * pGuild) {}
	virtual void lOnDeath(Player* pPlayer) {}
	virtual bool lOnRepop(Player* pPlayer) { return false; }
	virtual void lOnEmote(Player* pPlayer, uint32 Emote, Unit* pUnit) {}
	virtual void lOnEnterCombat(Player* pPlayer, Unit* pTarget) {}
	virtual bool lOnCastSpell(Player* pPlayer, SpellEntry * pSpell) { return false; }
	virtual bool lOnLogoutRequest(Player* pPlayer) { return false; }
	virtual void lOnLogout(Player* pPlayer) {}
	virtual void lOnQuestAccept(Player* pPlayer, Quest * pQuest) {}
	virtual void lOnZone(Player* pPlayer, uint32 Zone) {}
	virtual bool lOnChat(Player* pPlayer, uint32 Type, uint32 Lang, string Message, string Misc) { return false; }
	virtual void lOnLoot(Player* pPlayer, Unit* pTarget, uint32 Money, uint32 ItemId) {}
	virtual void lOnEnterWorld2(Player* pPlayer) {}
	virtual void lOnCharacterCreate(Player* pPlayer) {}
	virtual void lOnQuestCancelled(Player* pPlayer, Quest * pQuest) {}
	virtual void lOnQuestFinished(Player* pPlayer, Quest * pQuest) {}
	virtual void lOnHonorableKill(Player* pPlayer, Player* pKilled) {}
	virtual void lOnArenaFinish(Player* pPlayer, ArenaTeam* pTeam, bool victory, bool rated) {}
	virtual void lOnObjectLoot(Player* pPlayer, Object* pTarget, uint32 Money, uint32 ItemId) {}
	virtual void lOnAreaTrigger(Player* pPlayer, uint32 areaTrigger) {}
	virtual void lOnPostLevelUp(Player* pPlayer) {}
	virtual void lOnMountFlying(Player* plr) {}
	virtual void lOnPreAuraRemove(Player* plr,uint32 spellID) {}
};
#endif
