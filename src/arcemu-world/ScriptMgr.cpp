...
void ScriptMgr::UnloadScripts()
{
	...
	for(LuaHookMap::iterator itr = lua_hookmap->begin(); itr != lua_hookmap->end(); itr++)
	{
			delete (*itr);
	}
	lua_hookmap->clear();
...
#ifdef ENABLE_LUA_HOOKS
void ScriptMgr::restart_lua_scripts()
{
	for(LibraryHandleMap::iterator itr = _handles.begin(); itr != _handles.end(); itr++)
	{
#ifdef WIN32
		exp_script_register rcall = (exp_script_register)GetProcAddress((HMODULE)(*itr),"_exp_script_restart");
		if(rcall != NULL)
		{
			rcall(this);
			break;
...
#else
		exp_script_register rcall = (exp_script_register)dlsym( (HMODULE)(*itr),"_exp_script_restart");
		if(rcall != NULL)
		{
			rcall(this);
			break;
...
#ifdef ENABLE_LUA_HOOKS
void ScriptMgr::register_lua_hook(LuaHookInterface * pInterface)
{
	LuaHookMap::iterator itr = lua_hookmap->find(pInterface);
	if(itr == lua_hookmap->end() )
		lua_hookmap->insert(pInterface);
	else
	{
		delete (*itr);
		lua_hookmap->erase( itr);
		lua_hookmap->insert(pInterface);
	}
}
void ScriptMgr::unregister_lua_hook(LuaHookInterface *pInterface)
{
	LuaHookMap::iterator itr = lua_hookmap->find(pInterface);
	if(itr == lua_hookmap->end() )
		return;
...
#define OUTER_LOOP_BEGIN(type, fptr_type) if(!sScriptMgr._hooks[type].size()) {} else {\
	fptr_type call; \
...
#define OUTER_LOOP_BEGIN_COND(type, fptr_type) fptr_type call; \
	bool  ret_val = true; \
	if(!sScriptMgr._hooks[type].size()){ } else {\
	for(ServerHookList::iterator itr = sScriptMgr._hooks[type].begin(); itr != sScriptMgr._hooks[type].end(); itr++) { \
		call = ((fptr_type)*itr);

#define OUTER_LOOP_END_COND }}
...
#ifdef ENABLE_LUA_HOOKS
	for(LuaHookMap::iterator itr = sScriptMgr.lua_hookmap->begin(); itr != sScriptMgr.lua_hookmap->end(); itr++)
	{
		(*itr)->GetBoolResultMap().insert(make_pair(SERVER_HOOK_EVENT_ON_NEW_CHARACTER,ret_val));
	}
	CALL_LUA_HOOK(lOnNewCharacter(Race,Class,Session,Name))
#endif
	OUTER_LOOP_END_COND
	return ret_val;
}
...
void HookInterface::OnKillPlayer(Player* pPlayer, Player* pVictim)
{
	...
	OUTER_LOOP_END
#ifdef ENABLE_LUA_HOOKS
		CALL_LUA_HOOK(lOnKillPlayer(pPlayer,pVictim))
#endif
}
...
{
#ifdef ENABLE_LUA_HOOKS
	CALL_LUA_HOOK(lOnFirstEnterWorld(pPlayer))
#endif
...
{
#ifdef ENABLE_LUA_HOOKS
	CALL_LUA_HOOK(lOnCharacterCreate(pPlayer))
...
{
#ifdef ENABLE_LUA_HOOKS
	CALL_LUA_HOOK(lOnEnterWorld(pPlayer))
#endif // ENABLE_LUA_HOOKS
...
{
#ifdef ENABLE_LUA_HOOKS
	CALL_LUA_HOOK(lOnGuildCreate(pLeader,pGuild))
#endif // ENABLE_LUA_HOOKS
...
{
#ifdef ENABLE_LUA_HOOKS
	CALL_LUA_HOOK(lOnGuildJoin(pPlayer,pGuild))
#endif // ENABLE_LUA_HOOKS
...
{
#ifdef ENABLE_LUA_HOOKS
	CALL_LUA_HOOK(lOnDeath(pPlayer))
#endif // ENABLE_LUA_HOOKS
...
#ifdef ENABLE_LUA_HOOKS
	CALL_LUA_HOOK(lOnRepop(pPlayer))
		for(LuaHookMap::iterator itr = sScriptMgr.lua_hookmap->begin(); itr != sScriptMgr.lua_hookmap->end(); itr++)
		{
			(*itr)->GetBoolResultMap().insert(make_pair(SERVER_HOOK_EVENT_ON_REPOP,ret_val));
		}
#endif // ENABLE_LUA_HOOKS
	OUTER_LOOP_END_COND
	return ret_val;
...
{
#ifdef ENABLE_LUA_HOOKS
	CALL_LUA_HOOK(lOnEmote(pPlayer,Emote,pUnit))
#endif
...
bool HookInterface::OnCastSpell(Player* pPlayer, SpellEntry* pSpell)
{
	...
	OUTER_LOOP_END_COND
		...
		for(LuaHookMap::iterator itr = sScriptMgr.lua_hookmap->begin(); itr != sScriptMgr.lua_hookmap->end(); itr++)
		{
			(*itr)->GetBoolResultMap().insert(make_pair(SERVER_HOOK_EVENT_ON_CAST_SPELL,ret_val));
		}
		CALL_LUA_HOOK(lOnCastSpell(pPlayer, pSpell))
#endif
	return ret_val;
}
...
bool HookInterface::OnLogoutRequest(Player* pPlayer)
{
	...
	OUTER_LOOP_END_COND
		...
		for(LuaHookMap::iterator itr = sScriptMgr.lua_hookmap->begin(); itr != sScriptMgr.lua_hookmap->end(); itr++)
		{
			(*itr)->GetBoolResultMap().insert(make_pair(SERVER_HOOK_EVENT_ON_LOGOUT_REQUEST,ret_val));
		}
		CALL_LUA_HOOK(lOnLogoutRequest(pPlayer))
#endif // ENABLE_LUA_HOOKS
	return ret_val;
}
...
{
#ifdef ENABLE_LUA_HOOKS
	CALL_LUA_HOOK(lOnLogout(pPlayer))
#endif
...
{
#ifdef ENABLE_LUA_HOOKS
	CALL_LUA_HOOK(lOnQuestAccept(pPlayer,pQuest))
#endif
...
{
#ifdef ENABLE_LUA_HOOKS
	CALL_LUA_HOOK(lOnZone(pPlayer,Zone))
#endif
...
bool HookInterface::OnChat(Player* pPlayer, uint32 Type, uint32 Lang, string Message, string Misc)
{
	...
	ret_val = (call)(pPlayer, Type, Lang, Message, Misc);
 	OUTER_LOOP_END_COND
		...
		CALL_LUA_HOOK(lOnChat(pPlayer,Type,Lang,Message,Misc))
#endif
	return ret_val;
}
...
void HookInterface::OnLoot(Player* pPlayer, Unit* pTarget, uint32 Money, uint32 ItemId)
{
	...
	OUTER_LOOP_END
#ifdef ENABLE_LUA_HOOKS
		CALL_LUA_HOOK(lOnLoot(pPlayer,pTarget,Money,ItemId))
#endif
}
...
void HookInterface::OnObjectLoot(Player* pPlayer, Object* pTarget, uint32 Money, uint32 ItemId)
{
	...
	OUTER_LOOP_END
#ifdef ENABLE_LUA_HOOKS
		CALL_LUA_HOOK(lOnObjectLoot(pPlayer,pTarget,Money,ItemId))
#endif
}
...
void HookInterface::OnEnterWorld2(Player* pPlayer)
{
	OUTER_LOOP_BEGIN(SERVER_HOOK_EVENT_ON_ENTER_WORLD_2, tOnEnterWorld)
 	...
 	OUTER_LOOP_END
#ifdef ENABLE_LUA_HOOKS
		CALL_LUA_HOOK(lOnEnterWorld2(pPlayer))
#endif
}
...
void HookInterface::OnQuestCancelled(Player* pPlayer, Quest * pQuest)
{
	...
	OUTER_LOOP_END
#ifdef ENABLE_LUA_HOOKS
		CALL_LUA_HOOK(lOnQuestCancelled(pPlayer,pQuest))
#endif
}
...
void HookInterface::OnQuestFinished(Player* pPlayer, Quest * pQuest, Object* pObject)
{
	...
	OUTER_LOOP_END
#ifdef ENABLE_LUA_HOOKS
		CALL_LUA_HOOK(lOnQuestFinished(pPlayer,pQuest))
#endif
 }
...
void HookInterface::OnHonorableKill(Player* pPlayer, Player* pKilled)
{
	...
	OUTER_LOOP_END
#ifdef ENABLE_LUA_HOOKS
		CALL_LUA_HOOK(lOnHonorableKill(pPlayer,pKilled))
#endif
}
...
void HookInterface::OnArenaFinish(Player* pPlayer, uint32 type, ArenaTeam* pTeam, bool victory, bool rated)
{
	...
	OUTER_LOOP_END
#ifdef ENABLE_LUA_HOOKS
		CALL_LUA_HOOK(lOnArenaFinish(pPlayer,pTeam,victory,rated))
#endif
}
...
{
#ifdef ENABLE_LUA_HOOKS
	CALL_LUA_HOOK(lOnAreaTrigger(pPlayer,areatrigger))
#endif
...
bool HookInterface::OnMountFlying(Player* plr)
{
	...
	OUTER_LOOP_END_COND
		...
		for(LuaHookMap::iterator itr = sScriptMgr.lua_hookmap->begin(); itr != sScriptMgr.lua_hookmap->end(); itr++)
		{
			(*itr)->GetBoolResultMap().insert(make_pair(SERVER_HOOK_EVENT_ON_MOUNT_FLYING,ret_val));
		}
		CALL_LUA_HOOK(lOnMountFlying(plr))
#endif
	return ret_val;
}
...
bool HookInterface::OnPreAuraRemove(Player* plr,uint32 spellID)
{
	...
	OUTER_LOOP_END_COND
		...
		for(LuaHookMap::iterator itr = sScriptMgr.lua_hookmap->begin(); itr != sScriptMgr.lua_hookmap->end(); itr++)
		{
			(*itr)->GetBoolResultMap().insert(make_pair(SERVER_HOOK_EVENT_ON_PRE_AURA_REMOVE,ret_val));
		}
		CALL_LUA_HOOK(lOnPreAuraRemove(plr,spellID))
#endif
	return ret_val;
}
...
{
#ifdef ENABLE_LUA_HOOKS
	CALL_LUA_HOOK(lOnPostLevelUp(pPlayer))
#endif
