...
class UnworthyInitiate : public CreatureAIScript
{
	public:
	ADD_CREATURE_FACTORY_FUNCTION(UnworthyInitiate);
	UnworthyInitiate(Creature* pCreature) : CreatureAIScript(pCreature)
		{
		...
		void OnCombatStart(Unit* mTarget)
			{
				switch(rand()%8)
				{
			case 0: _unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, SAY_ATTACK_1); break;
			case 1: _unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, SAY_ATTACK_2); break;
			case 2: _unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, SAY_ATTACK_3); break;
			case 3: _unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, SAY_ATTACK_4); break;
			case 4: _unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, SAY_ATTACK_5); break;
			case 5: _unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, SAY_ATTACK_6); break;
			case 6: _unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, SAY_ATTACK_7); break;
			case 7: _unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, SAY_ATTACK_8); break;
			case 8: _unit->SendChatMessage(CHAT_MSG_MONSTER_YELL, LANG_UNIVERSAL, SAY_ATTACK_9); break;
				}
		...
		void AIUpdate()
		{
			...
			if( state == PHASE_INACTIVE )
			{
				...
				Index = mTimerIdCounter++;
				mTimers.push_back(std::make_pair(Index, 1000));
			//	return Index;
			//	timer = AddTimer(1000);
				state = PHASE_STANDUP;
			}
			else if( state == PHASE_STANDUP && (Index) )
			{
				_unit->SendChatMessage(CHAT_MSG_MONSTER_SAY, LANG_UNIVERSAL, "You have committed a big mistake, demon");
				Index = mTimerIdCounter++;
				mTimers.push_back(std::make_pair(Index, 5000));
			//	timer = AddTimer(5000);
				state = PHASE_REACH_ARMORY;
			}
			else if( state == PHASE_REACH_ARMORY && (Index) )
			{
				...
				_unit->GetAIInterface()->MoveTo(m_ArmoryLocations[nearest_entry].x, m_ArmoryLocations[nearest_entry].y, m_ArmoryLocations[nearest_entry].z, true);
				Index = mTimerIdCounter++;
				mTimers.push_back(std::make_pair(Index, 2000));
			//	timer = AddTimer(2000);
				state = PHASE_ACTIVATE;
			}
			else if( state == PHASE_ACTIVATE && (Index) )
			{
				// face off the player
				//Player * plr = _unit->GetMapMgrPlayer( _unit->GetSummonedByGUID() );
				//if( plr )
			...
			else if( state == PHASE_ATTACK_PLAYER && (Index) )
			{
			...
			else if( state == PHASE_ATTACKING && (Index) )
			{
				//_unit->SetFaction( 16 );
...
void SetupDeathKnight(ScriptMgr* mgr)
{
	...
	mgr->register_dummy_spell(51859, &SiphonofAcherus);
