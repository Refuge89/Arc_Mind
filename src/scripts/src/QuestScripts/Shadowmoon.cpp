...
//WP LocationExtra Wait Times
struct WPWaitTimes{
	LocationExtra mLocationExtra; 
	uint32 WaitTime;
};
...
public:
	...
	DeathbringerJovaanAI(Creature* pCreature) : MoonScriptCreatureAI(pCreature)
	{
		...
		for (int i = 1; i < 5; ++i)
		{
			AddWaypoint(CreateWaypoint(i, DeathbringerJovaanWP[i].WaitTime, DeathbringerJovaanWP[i].mLocationExtra.mAddition, DeathbringerJovaanWP[i].mLocationExtra));
		}	
	}
