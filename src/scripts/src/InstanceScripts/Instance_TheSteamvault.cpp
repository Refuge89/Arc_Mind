...
static LocationExtra SpawnLocationExtra[] =
{
...
public:
    ...
    void AIUpdate()
    {
		...
		if ((_unit->GetHealthPct() <= 75 && GnomeCounter == 0) || (_unit->GetHealthPct() <= 50 && GnomeCounter == 1) || (_unit->GetHealthPct() <= 25 && GnomeCounter == 2))
        {
            ...
            for(int i = 0; i < 3; i++)
            {
                Gnome = _unit->GetMapMgr()->GetInterface()->SpawnCreature(CN_STEAMRIGGER_MECHANIC, SpawnLocationExtra[i].x, SpawnLocationExtra[i].y, SpawnLocationExtra[i].z, SpawnLocationExtra[i].o, true, false, _unit->GetUInt32Value(UNIT_FIELD_FACTIONTEMPLATE), 50);
                if(Gnome)
                {
					Gnome->GetAIInterface()->SetUnitToFollow(_unit);
					Gnome->GetAIInterface()->SetFollowDistance(15.0f);
...
static LocationExtra Distiller[]=
{
	{  },
...
static LocationExtra DistillerMoveTo[]=
{
	{  },
