...
void DarkPortalEventTriggerAI::AIUpdate()
{
	...
	if ( IsTimerFinished( mInfernalSummonTimer ) )
	{
		...
		else
		{
			...
			{
				InfernalSiegebreakerAI* InfernalAI = static_cast< InfernalSiegebreakerAI* >( SpawnCreature( CN_INFERNAL_SIEGEBREAKER, Infernals[ mInfernalSpawnPointId + i * 2 ].x, Infernals[ mInfernalSpawnPointId + i * 2 ].y, Infernals[ mInfernalSpawnPointId + i * 2 ].z, Infernals[ mInfernalSpawnPointId + i * 2 ].o ) );
				if ( InfernalAI != NULL )
				{
...
void DarkPortalEventTriggerAI::SpecialUnitChecks()
{
	...
	if ( mJusti == NULL )
	{
		...
		mJusti = static_cast< JustiniusTheHarbingerAI* >( SpawnCreature( CN_JUSTINIUS_THE_HARBINGER, AHSPoints[ 0 ].x, AHSPoints[ 0 ].y, AHSPoints[ 0] .z, AHSPoints[ 0 ].o ) );
		if ( mJusti != NULL )
		{
	...
	if ( mMelgr == NULL )
	{
		...
		mMelgr = static_cast< MelGrommHighmountainAI* >( SpawnCreature( CN_MELGROMM_HIGHMOUNTAIN, AHSPoints[ 1 ].x, AHSPoints[ 1 ].y, AHSPoints[ 1 ].z, AHSPoints[ 1 ].o ) );
		if ( mMelgr != NULL )
		{
...
void DarkPortalEventTriggerAI::SpawnAllianceUnit()
{
	...
	LocationExtra* UnitLocationExtra = NULL;
	...
	for ( uint32 i = 0; i < AIList->size(); ++i )
	{
		...
		if ( SetUnit == NULL || !SetUnit->IsAlive() || !SetUnit->GetUnit()->IsInWorld() )
		{
			SetUnit = mAlliance = SpawnCreature( Entry, ASPoints[ 0 ].x, ASPoints[ 0 ].y, ASPoints[ 0 ].z, ASPoints[ 0 ].o );
			...
			if ( SetUnit != NULL )
			{
				...
				for ( uint32 x = 1; x < 11; ++x )
				{
					if ( x != 10 )
						...
						NewWaypoint = CreateWaypoint( x, 0, Flag_Run, UnitLocationExtra[ i ] );
...
void DarkPortalEventTriggerAI::SpawnHordeUnit()
{
	...
	for ( uint32 i = 0; i < AIList->size(); ++i )
	{
		...
		if ( SetUnit == NULL || !SetUnit->IsAlive() || !SetUnit->GetUnit()->IsInWorld() )
		{
			SetUnit = mHorde = SpawnCreature( Entry, HSPoints[ 0 ].x, HSPoints[ 0 ].y, HSPoints[ 0 ].z, HSPoints[ 0 ].o );
...
void DarkPortalEventTriggerAI::PitCommanderInfernalSummon()
{
	...
	if ( mPitCommander != NULL && mInfernalTable.size() <= 2 )
	{
		...
		_unit->CastSpellAoF( Infernals[ i ].x, Infernals[ i ].y, Infernals[ i ].z, dbcSpell.LookupEntry( DARK_PORTAL_EVENT_TRIGGER_SUMMON_INFERNALS2 ), true );
		_unit->CastSpellAoF( Infernals[ i + 2 ].x, Infernals[ i + 2 ].y, Infernals[ i + 2 ].z, dbcSpell.LookupEntry( DARK_PORTAL_EVENT_TRIGGER_SUMMON_INFERNALS2 ), true );
...
void DarkPortalEventTriggerAI::PitCommanderSpawn()
{
	...
	for ( uint32 i = 0; i < 10; i += WaveSize )
	{
		...
		MoonScriptCreatureAI* ScriptAI = SpawnCreature( Entry, DSPoints[ i ].x, DSPoints[ i ].y, DSPoints[ i ].z, DSPoints[ i ].o );
		if ( ScriptAI != NULL )
		{
			// Crappy method
			switch ( Entry )
