...
void SetupSunwellPlateau(ScriptMgr* mgr)
{
	...
	mgr->register_creature_script(CN_KALECGOS, &SUNWELL_KALECGOS::DRAGON_KALECGOS::Create);
	mgr->register_creature_script(CN_SATHROVAR, &SUNWELL_KALECGOS::SATHROVAR::Create);
	mgr->register_creature_script(CN_DARK_ELF, &SUNWELL_KALECGOS::DARK_ELF::Create);
	mgr->register_dummy_spell(KALECGOS_SPECTRAL_TELEPORT, &SUNWELL_KALECGOS::HandleSpectralTeleport);
	mgr->register_creature_script(CN_BRUTALLUS, &BrutallusAI::Create);
	mgr->register_creature_script(CN_FELMYST, &FelmystAI::Create);
	mgr->register_creature_script(CN_LADY_SACROLASH, &LadySacrolashAI::Create);
	mgr->register_creature_script(CN_GRAND_WARLOCK_ALYTHESS, &GrandWarlockAlythessAI::Create);
	mgr->register_creature_script(CN_MURU, &MuruAI::Create);
	mgr->register_creature_script(CN_SHADOWSWORD_BERSERKER, &ShadowswordBerserkerAI::Create);
	mgr->register_creature_script(CN_SHADOWSWORD_FURY_MAGE, &ShadowswordFuryMageAI::Create);
	mgr->register_creature_script(CN_VOID_SENTINEL, &VoidSentinelAI::Create);
	mgr->register_creature_script(CN_ENTROPIUS, &EntropiusAI::Create);
	mgr->register_creature_script(CN_KILJAEDEN, &KilJaedenAI::Create);
	mgr->register_creature_script(CN_HAND_OF_THE_DECEIVER, &HandOfTheDeceiverAI::Create);
}
