...
public:
	...
	KaelThasAI(Creature* pCreature) : MoonScriptBossAI(pCreature)
	{
		...
		for (int i = 0; i < 4; ++i)
		{
			Creature* pCreature = TO_CREATURE(ForceCreatureFind(Advisors[i].mAddition, Advisors[i].x, Advisors[i].y, Advisors[i].z));
			if (pCreature != NULL)
			{
				pCreature->Despawn(0, 0);
			}

			SpawnCreature(Advisors[i].mAddition, Advisors[i].x, Advisors[i].y, Advisors[i].z, Advisors[i].o);
		}
		for (int i = 0; i < 2; ++i)
		{
			GameObject* pGameobject = _unit->GetMapMgr()->GetInterface()->GetGameObjectNearestCoords(Gates[i].x, Gates[i].y, Gates[i].z, Gates[i].mAddition);
	...
	void CheckAdvisorState(Creature* pCreature)
	{
		...
		{
			LocationExtra pLocationExtra;
			pLocationExtra.x = pCreature->GetPositionX();
			pLocationExtra.y = pCreature->GetPositionY();
			pLocationExtra.z = pCreature->GetPositionZ();
			pLocationExtra.o = pCreature->GetOrientation();
			pLocationExtra.mAddition = pCreature->GetEntry();

			SetAIUpdateFreq(5000);
			SetPhase(GetPhase() + 1);
			mAdvLocationExtra.push_back(pLocationExtra);
			mAdvisorPhase = PHASE_SPEECH;
		}
	}
	...
	void AIUpdate()
	{
		if (GetPhase() < 5)
		{
			...
			{
				LocationExtra pLocationExtra;
				pLocationExtra.x = Advisors[i].x;
				pLocationExtra.y = Advisors[i].y;
				pLocationExtra.z = Advisors[i].z;
				pLocationExtra.o = Advisors[i].o;
				pLocationExtra.mAddition = Advisors[i].mAddition;
				mAdvLocationExtra.push_back(pLocationExtra);
		...
		if (GetPhase() == 5)
		{
			...
			else if (mEventTimer == -2)
			{
				for (int i = 0; i < 7; ++i)
				{
					SpawnCreature(KaelthasWeapons[i].mAddition, KaelthasWeapons[i].x, KaelthasWeapons[i].y, KaelthasWeapons[i].z, KaelthasWeapons[i].o);
				}
			...
			else if (IsTimerFinished(mEventTimer))
			{
				for (int i = 0; i < 4; ++i)
				{
					if (mAdvLocationExtra.size() <= (size_t)i)
						break;

					Creature* pCreature = TO_CREATURE(ForceCreatureFind(mAdvLocationExtra[i].mAddition, mAdvLocationExtra[i].x, mAdvLocationExtra[i].y, mAdvLocationExtra[i].z));
					if (pCreature != NULL && !pCreature->isAlive())
					{
						pCreature->Despawn(0, 0);
					}

					MoonScriptCreatureAI *pAI = SpawnCreature(mAdvLocationExtra[i].mAddition, mAdvLocationExtra[i].x, mAdvLocationExtra[i].y, mAdvLocationExtra[i].z, false);
					if (pAI != NULL)
					{
	...
	std::vector<LocationExtra>	mAdvLocationExtra;
};
