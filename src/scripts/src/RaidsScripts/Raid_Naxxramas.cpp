...
/*void SpellFunc_AnubRekhanCorpseScarabsPlayer( SpellDesc* pThis, MoonScriptCreatureAI* pCreatureAI, Unit * pTarget, TargetType pType )
{
	...
	if ( AnubRekhan != NULL )
	{
		std::vector< pair< Player *, LocationExtra > > PlayerCorpses;
		...
		{ 
			...
			LocationExtra ObjectLocationExtra;
			ObjectLocationExtra.x = ObjectPtr->GetPositionX();
			ObjectLocationExtra.y = ObjectPtr->GetPositionY();
			ObjectLocationExtra.z = ObjectPtr->GetPositionZ();
			ObjectLocationExtra.o = ObjectPtr->GetOrientation();
			PlayerCorpses.push_back( make_pair( PlayerPtr, ObjectLocationExtra ) );
		};
