...
static float ShieldGeneratorLocationExtra[4][3] =
{
...
public:
	...
	void OnReachWP(uint32 iWaypointId, bool bForwards)
	{
		...
		case 1:
			...
			for(uint8 i = 0; i < 4; i++)
			{
				channel = _unit->GetMapMgr()->GetInterface()->SpawnCreature(CN_SHIELD_GENERATOR_CHANNEL, ShieldGeneratorLocationExtra[i][0],  ShieldGeneratorLocationExtra[i][1],  ShieldGeneratorLocationExtra[i][2], 0, true, false, 0, 0);
				if(channel)
				{
