...
class SCRIPT_DECL EasyFunctions
{
	public:
		...
		bool AddItem(uint32 pEntry, Player* pPlayer, uint32 pCount = 1)
		{
			...
			if(ItemStack == NULL)
			{
				...
				else
				{
					...
					pPlayer->GetSession()->SendItemPushResult(NewItem,false, true, true, true,
					                            pPlayer->GetItemInterface()->LastSearchItemBagSlot(), pPlayer->GetItemInterface()->LastSearchItemSlot(), pCount);
					                  //          NewItem->GetEntry(), NewItem->GetItemRandomSuffixFactor(), NewItem->GetItemRandomPropertyId(), NewItem->GetCount());
					return true;
				};
			}
			else
			{
				...
				pPlayer->GetSession()->SendItemPushResult(ItemStack,false, true, true, false, static_cast<uint8>(pPlayer->GetItemInterface()->GetBagSlotByGuid(ItemStack->GetGUID())), 0xFFFFFFFF,  pCount );// ItemStack->GetEntry(), ItemStack->GetItemRandomSuffixFactor(), ItemStack->GetItemRandomPropertyId(), ItemStack->GetStackCount());
				return true;
			};
		...
		{
			sEventMgr.AddEvent(creat, &Creature::Destructor, EVENT_CREATURE_SAFE_DELETE, time, 1, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
		}
