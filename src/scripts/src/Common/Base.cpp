...
void MoonScriptCreatureAI::CastOnAllInrangePlayers(uint32 pSpellId, bool pTriggered)
{
	...
	{
		_unit->CastSpell(TO_PLAYER(*PlayerIter), pSpellId, pTriggered);
	};
};
...
//	return (NearestCreature) ? TO_MOONSCRIPTCREATUREAI(NearestCreature->GetScript()) : NULL;
...
//	MoonScriptCreatureAI* CreatureScriptAI = (NewCreature) ? TO_MOONSCRIPTCREATUREAI(NewCreature->GetScript()) : NULL;
...
void MoonScriptCreatureAI::RemoveAuraOnPlayers(uint32 pSpellId)
{
	...
	{
		// need testing
	//	(TO_PLAYER(*PlayerIter))->RemoveAura(pSpellId);
...
void EventFunc_ChangeGoState(MoonScriptCreatureAI* pCreatureAI, int32 pMiscVal)
{
	...
	uint32 pGOEntry = static_cast< uint32 >(pMiscVal);
		...
		UnitArray Array;
		for ( uint32 i = 0; i != pInstance->m_GOHighGuid; ++i )
		{
			pSelectedGO = pInstance->m_gameObjectStorage[ i ];
		if(pSelectedGO->GetEntry() == pGOEntry)
		{
			pSelectedGO->SetState(pSelectedGO->GetState() == 1 ? 0 : 1);
		}
		};
/*	for(std::vector< GameObject* >::iterator GOIter = pInstance->m_gameObjectStorage.begin(); GOIter != pInstance->m_gameObjectStorage.end(); ++GOIter)
	{
		pSelectedGO = (*GOIter);
		if(pSelectedGO->GetEntry() == pGOEntry)
		{
